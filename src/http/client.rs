...
impl PantosHttpClient {
    ...
    pub async fn identify_repesentative_invoice_barcode(&self, tracking_number: &str) -> GetIdentifyRepresentativeInvoiceBarcodeResponse {
        ...
        if response.status().is_success() {
            ...
            match response.json::<ErrorResponse>().await {
                Ok(err_msg) => panic!(
                    "[FLODY_CONSOLE] | [RES] 대표 송장 바코드 인식 error: {err_msg}",
                ),
                Err(err) => {
                    panic!(
    ...
    pub async fn worker_arrived(&self, picking_ids: &Vec<String>) {
        println!("[FLODY_CONSOLE] | [REQ] 작업자 도착 요청 picking_ids={:?}", picking_ids);
            ...
            .await
        {
            Ok(_res) => match _res.error_for_status() {
                ...
                Err(_err) => panic!("[FLODY_CONSOLE] | [RES] 작업자 도착 error: {_err} | body: {body}"),
            },
            Err(_err) => panic!("[FLODY_CONSOLE] | [RES] 작업자 도착 failed due to: {_err} | body: {body}"),
        }
    }
    ...
    pub async fn get_same_location_pickings(&self, picking_id: &str) -> Vec<PickingDto> {
        ...
        if response.status().is_success() {
            ...
            match response.json::<ErrorResponse>().await {
                Ok(err_msg) => panic!(
                    "[FLODY_CONSOLE] | [RES] 동일 로케이션 피킹 error: {err_msg}",
                ),
                Err(err) => {
                    panic!(
    ...
    pub async fn get_all_assigned_pickings(&self, robot_uid: &str) -> Vec<GetAllAssignedPickingsResponse> {
        ...
        if response.status().is_success() {
            ...
            match response.json::<ErrorResponse>().await {
                Ok(err_msg) => panic!(
                    "[FLODY_CONSOLE] | [RES] 작업 목록 확인 error: {err_msg}",
                ),
                Err(err) => {
                    panic!(
    ...
    pub async fn get_total_unloadings(&self) -> GetUnloadingsTotalResponse {
        ...
        if response.status().is_success() {
            ...
            match response.json::<ErrorResponse>().await {
                Ok(err_msg) => panic!(
                    "[FLODY_CONSOLE] | [RES] 전체 언로딩 정보 error: {err_msg}",
                ),
                Err(err) => {
                    panic!(
    ...
    pub async fn set_robot_status_idle(&self, robot_uid: &str) {
        ...
        match self.client.post(url).headers(headers).send().await {
            Ok(_res) => match _res.error_for_status() {
                ...
                Err(_err) => panic!(
                    "[FLODY_CONSOLE] | [RES] 로봇 대기 response error: {}",
                    _err
                ),
            },
            Err(_err) => panic!(
                "[FLODY_CONSOLE] | [RES] 로봇 대기 failed due to: {}",
                _err
            ),
        }
    }
